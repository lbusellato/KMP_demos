...
class KMP:
    ...
    """
    def __init__(self, l=1, lc=60, tol=0.002, kernel_delta=0.001, kernel_gamma=6) -> None:
    ...
    def fit(self, X, Y, var):
        ...
        for i in range(self.N):
            for j in range(i+1):
                k = self.__kernel_matrix(self.s[:,i],self.s[:,j])
                kernel_mean = k
                kernel_covariance = k
                if i == j:
                    kernel_mean = kernel_mean + self.l*self.sigma[:,:,i]
                    kernel_covariance = kernel_covariance + self.lc*self.sigma[:,:,i]
                k_mean[i*self.O:(i+1)*self.O,j*self.O:(j+1)*self.O] = kernel_mean
                k_covariance[i*self.O:(i+1)*self.O,j*self.O:(j+1)*self.O] = kernel_covariance
                k_mean[j*self.O:(j+1)*self.O,i*self.O:(i+1)*self.O] = kernel_mean
                k_covariance[j*self.O:(j+1)*self.O,i*self.O:(i+1)*self.O] = kernel_covariance
